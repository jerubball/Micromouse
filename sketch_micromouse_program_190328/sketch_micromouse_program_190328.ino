/**********************************************************\
 *  Micromouse. Mouse Control Program.                    * 
 *    Version 1.7.30                                      * 
 *    2019MAR28THU1700EDT                                 * 
 ...
 *      Each in Analog In pins 3, 4, 5.
 *      Respective positions are: Left, Front, Right.
 *    2 DFRobot FIT0441 Brushless DC Motors with Encoder.
 *      Each in Digital pins 4, 5, 6, and 8, 9, 10.
 *      Respective wire colors are: Blue, Yellow, Green.
 *    1 Arduino Built-in LED for visual feedback.
 *      Defaults to Digital pin 13.
 *    1 CEM-1203(42) Magnetic Buzzer for auditory feedback.
 *      Analog In pin 0 for Arduino Zero Digital-Analog Converter pin.
 *    2 Push-to-open switches. Red and Blue.
 *      Each in Digital pins 2 and 3.
 *    2 Toggle on/off switches. Upper and Lower.
 *      Each in Digital pins 11 and 12.
 *  
 *  Instruction Overview.
 *     
 *     1. Make sure two buttons are connected and not pressed.
 *        Long-delay short-blink (200 ms on, 800 ms off) indicates attention.
 *        NOTE: For next stages, position front sensor to face you.
 *        Blue button will be on left side, Red button will be on right side.
 *        Two switches combine to create 2-bit binary number, left switch for upper bit and right switch for lower bit.
 *        0: (Blue)   [ *]   [ *]   (Red)
 *        1: (Blue)   [ *]   [* ]   (Red)
 *        2: (Blue)   [* ]   [ *]   (Red)
 *        3: (Blue)   [* ]   [* ]   (Red)
 ...
 *        Press and hold both buttons to instantly play music.
 *     
 *     3. Adjust switch to choose starting position.
 *        Position is indicated by (x, y) coordinate of switch.
 *        0: (0, 0) South-West corner.
 *        1: (0, 1) South-East corner.
 *        2: (1, 0) North-West corner.
 *        3: (1, 1) North-East corner.
 ...
 *        Short-delay short-blink (100 ms on, 400 ms off) will appear 4 times (2 seconds).
 *        Press and hold both buttons to enter sensor debug mode.
 *        0: exit debug mode.
 *        1: Sound buzzer with left sensor.
 *        2: Sound buzzer with front sensor.
 *        3: Sound buzzer with right sensor.
 *     
 *     4. Adjust switch to choose starting direction.
 *        Direction is indicated by 90 degree increment of standard 0 degree.
 *        0: (0 degrees) Facing East.
 *        1: (90 degrees) Facing North.
 *        2: (180 degrees) Facing West.
 *        3: (270 degrees) Facing South.
 ...
 *        Short-delay short-blink (100 ms on, 400 ms off) will appear 4 times (2 seconds).
 *        Press and hold both buttons to enter switch debug mode.
 *        Buzzer sound of varying pitch (low for 0 to high for 3) will be ring.
 *     
 *     5. When program is about to start, short-delay equal-blink (250 ms on, 250 ms off) will appear 6 times (3 seconds).
 *        Adjust switch to set search priority.
 *        Upper-bit switch determines straight-turn priority, lower-bit switch determines left-right priority.
 *        0: [ *] [ *]  Do straight-left-right
 *        1: [ *] [* ]  Do straight-right-left
 *        2: [* ] [ *]  Do left-right-straight
 *        3: [* ] [* ]  Do right-left-straight
 *     
 *     6. During the execution of program, press and hold Blue button to issue save command.
 ...
 *     
 *     7. During the execution of program, press and hold Red button to issue stop command.
...
const byte program_test = 51, program_debug = 52;
...
void setup ();  // line 250
void loop ();  // line 300
void doEscapeLoop ();  // line 350
void doPreLoop ();  // line 400
void doPostLoop ();  // line 400

// Flow Control Definition
byte doProgramStartInit ();  // line 400
byte doProgramStartWait ();  // line 400
byte doProgramIntermediate ();  // line 450
byte doProgramPause ();  // line 500
byte doProgramStop ();  // line 500
byte doMovementTurn ();  // line 500
byte doMovementMove ();  // line 500
byte doMovementFix ();  // line 550
byte doSearchInit ();  // line 650
byte doSearchPlan ();  // line 650
byte doSolveInit ();  // line 750
byte doSolvePlan ();  // line 850
byte doReturnInit ();  // line 900
byte doReturnPlan ();  // line 950
byte doProgramTest ();  // line 1000
byte doProgramDebug (short stage); // line 1000

// Software Utility Definition
void readMaze ();  // line 1050
void writeMaze ();  // line 1050
void maskMaze (byte val);  // line 1050
unsigned short encodeLocation (byte x, byte y);  // line 1050
byte decodeLocationX (unsigned short loc);  // line 1050
byte decodeLocationY (unsigned short loc);  // line 1100
byte getCellType (byte x, byte y);  // line 1100
short getLocationDX (byte d);  // line 1100
short getLocationDY (byte d);  // line 1100
short getDirection (short x1, short y1, short x2, short y2);  // line 1100
bool checkGoalPost (byte x, byte y);  // line 1100
bool checkGoalCell (byte x, byte y);  // line 1150
unsigned short findGoalPost ();  // line 1150
void getNeighbors (bool neighbors[], byte x, byte y);  // line 1150
short signum (float val);  // line 1200
short signumOld (float val);  // line 1200

// Hardware Utility Definition
void zeroMotor (bool mode, short dur, byte rep);  // line 1200
void setMotor (float left, float right);  // line 1200
void setMotorEncoder (long left, long right);  // line 1250
void readMotorEncoderOld ();  // line 1250
void interruptMotorEncoderLeft ();  // line 1250
void interruptMotorEncoderRight ();  // line 1250
bool runMotorEncoder (float power, byte mode, float dist);  // line 1300
bool shiftMotorEncoder (float power, byte mode, float dist);  // line 1350
float readSensor (byte id);  // line 1400
bool readSensorDirection (byte dir);  // line 1400
byte readSwitches (byte upper_id, byte lower_id);  // line 1400
bool confirmButton (byte id, byte nid, bool mode, byte bid, short on, short off, byte rep); // line 1400
bool confirmButtons (byte id1, byte id2, bool mode, byte bid, short on, short off, byte rep); // line 1450
bool confirmButtonOld (byte id, bool mode, byte bid, short on, short off, byte rep); // line 1450
void blinkDelay (byte id, short on, short off, byte rep);  // line 1500
void buzzerSensorFeedback (byte id); // line 1500

// Sound Utility Definition
void ringBuzzer (byte id, float freq, short dur, float power);  // line 1500
float getFreq (short note);  // line 1550
void playMusic (byte code);  // line 1550
void playWeAreTheChampion (float power, short beat, short offset);  // line 1550
void playGonnaFlyNow (float power, short beat, short offset);  // line 1600
void playWeWillRockYou (float power, short beat, short offset); // line 1700
void playPsyChampion (float power, short beat, short offset); // line 1750
...
void doEscapeLoop () {
  ...
  if ((current_state > 2) && (digitalRead (button_red) == HIGH || digitalRead (button_blue) == HIGH)) {
    ...
    setMotor (0, 0);
    // write button
    if (confirmButton (button_blue, button_red, HIGH, LED_BUILTIN, 100, 400, 6)) { // short-delay short-blink
    ...
    // stop button
    if (confirmButton (button_red, button_blue, HIGH, LED_BUILTIN, 500, 500, 3)) { // long-delay equal-blink
      blinkDelay (LED_BUILTIN, 250, 250, 6); // short-delay equal-blink
      current_state = program_stop;
      blinkDelay (LED_BUILTIN, 250, 250, 6); // short-delay equal-blink
    }
...
byte doProgramStartWait () {
  ...
  for (byte stage = 0; stage < option_size;) {
    if (confirmButtons (button_red, button_blue, HIGH, LED_BUILTIN, 400, 100, 4)) { // short-delay long-blink
      delay (500);
      doProgramDebug (stage);
      delay (500);
    }
    else if (confirmButton (button_red, button_blue, HIGH, LED_BUILTIN, 500, 500, 2)) { // long-delay equal-blink
      options[stage++] = readSwitches (switch_upper, switch_lower);
      options[stage++] = true;
      ringBuzzer (buzzer, getFreq(12), 1000, music_power); // A1
    }
    else if (confirmButton (button_blue, button_red, HIGH, LED_BUILTIN, 100, 400, 4)) { // short-delay short-blink
      options[stage++] = readSwitches (switch_upper, switch_lower);
      options[stage++] = false;
      ringBuzzer (buzzer, getFreq(5), 1000, music_power); // D1
    }
  }
...
byte doProgramDebug (short stage) {
  byte choice = readSwitches (switch_upper, switch_lower);
  switch (stage) {
    case 0:
    case 1:
      // play music
    ...
    case 3:
      while (choice > 0) {
        buzzerSensorFeedback (ir_front + choice - 2);
        choice = readSwitches (switch_upper, switch_lower);
      }
      break;
    case 4:
    case 5:
      ringBuzzer (buzzer, getFreq(choice), 500, music_power);
      break;
    default:
      break;
  }
 ...
 * Cancel if other button is pressed.
 */
bool confirmButton (byte id, byte nid, bool mode, byte bid, short on, short off, byte rep) {
  if (digitalRead (id) != mode || digitalRead (nid) == mode) {
    return false;
  }
  else {
    for (byte i = 0; i < rep; i++) {
...
/**
 * Checks if both given pins are in selected mode for given duration with visual feedback.
 */
bool confirmButtons (byte id1, byte id2, bool mode, byte bid, short on, short off, byte rep) {
  if (digitalRead (id1) != mode || digitalRead (id2) != mode) {
    return false;
  }
  else {
    for (byte i = 0; i < rep; i++) {
 ...
 * Checks if given pin is in selected mode for given duration with visual feedback.
 */
bool confirmButtonOld (byte id, bool mode, byte bid, short on, short off, byte rep) {
...
/**
 * Do auditory feedback on Sensor.
 */
void buzzerSensorFeedback (byte id) {
  float value = readSensor (id);
  float freq = 440 * pow (2.0, music_offset / 12.0 + value * 2 - 1);
  ringBuzzer (buzzer, freq, music_beat, music_power);
  ringBuzzer (buzzer, 0, music_beat * 1, music_power);
}
...
void playMusic (byte code) {
  switch (code) {
    ...
    case 1:
      ...
      break;
    case 2:
      playWeWillRockYou (music_power, music_beat * 1.5, music_offset);
      break;
    case 3:
      playPsyChampion (music_power, music_beat * 1.5, music_offset);
      break;
    default:
      break;
  }
...
void playWeWillRockYou (float power, short beat, short offset) {
  ringBuzzer (buzzer, getFreq(4 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, getFreq(6 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, getFreq(6 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, getFreq(6 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, getFreq(6 + offset), beat * 1, power);
  ringBuzzer (buzzer, getFreq(6 + offset), beat * 0.5, power);
  ...
  ringBuzzer (buzzer, getFreq(4 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, getFreq(6 + offset), beat * 1, power);
  ringBuzzer (buzzer, getFreq(11 + offset), beat * 1, power);
  ringBuzzer (buzzer, getFreq(9 + offset), beat * 1, power);
  ringBuzzer (buzzer, getFreq(9 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, getFreq(4 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, getFreq(6 + offset), beat * 1, power);
  ringBuzzer (buzzer, getFreq(6 + offset), beat * 1, power);
  ringBuzzer (buzzer, getFreq(6 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, getFreq(4 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, getFreq(6 + offset), beat * 1, power);
  ringBuzzer (buzzer, getFreq(4 + offset), beat * 1, power);
  ringBuzzer (buzzer, getFreq(6 + offset), beat * 1, power);
  ringBuzzer (buzzer, getFreq(6 + offset), beat * 1, power);
  ringBuzzer (buzzer, getFreq(6 + offset), beat * 1, power);
  ringBuzzer (buzzer, 0, beat * 1, power);
  ...
  ringBuzzer (buzzer, getFreq(4 + offset), beat * 1, power);
  ringBuzzer (buzzer, getFreq(4 + offset), beat * 1, power);
  ringBuzzer (buzzer, getFreq(1 + offset), beat * 1, power);
  ringBuzzer (buzzer, getFreq(-1 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, getFreq(-3 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, getFreq(-6 + offset), beat * 1, power);
  ringBuzzer (buzzer, getFreq(-6 + offset), beat * 1, power);
  ...
  ringBuzzer (buzzer, getFreq(4 + offset), beat * 0.5, power);
  for (byte i = 0; i < 2; i++) {
  ringBuzzer (buzzer, getFreq(9 + offset), beat * 2, power);
  ringBuzzer (buzzer, getFreq(8 + offset), beat * 2, power);
  ringBuzzer (buzzer, getFreq(6 + offset), beat * 2, power);
  ringBuzzer (buzzer, getFreq(4 + offset), beat * 2, power);
  ringBuzzer (buzzer, getFreq(6 + offset), beat * 1, power);
  ringBuzzer (buzzer, getFreq(6 + offset), beat * 1, power);
  ringBuzzer (buzzer, 0, beat * 2, power);
  ringBuzzer (buzzer, getFreq(-6 + offset), beat * 1, power);
  ...
  ringBuzzer (buzzer, getFreq(4 + offset), beat * 2, power);
  ringBuzzer (buzzer, getFreq(6 + offset), beat * 1, power);
  ringBuzzer (buzzer, getFreq(6 + offset), beat * 2.5, power);
  ringBuzzer (buzzer, 0, beat * 0.5, power);
  ringBuzzer (buzzer, getFreq(6 + offset), beat * 1, power);
  ringBuzzer (buzzer, 0, beat * 2, power);
}

void playPsyChampion (float power, short beat, short offset) {
  for (byte i = 0; i < 2; i++) {
  ringBuzzer (buzzer, getFreq(0 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, 0, beat * 0.5, power);
  ringBuzzer (buzzer, getFreq(3 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, 0, beat * 0.25, power);
  ringBuzzer (buzzer, getFreq(0 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, 0, beat * 0.25, power);
  ringBuzzer (buzzer, getFreq(5 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, getFreq(3 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, getFreq(-2 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, getFreq(0 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, 0, beat * 0.5, power);
  ...
  ringBuzzer (buzzer, getFreq(0 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, 0, beat * 0.25, power);
  ringBuzzer (buzzer, getFreq(8 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, getFreq(7 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, getFreq(3 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, getFreq(0 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, getFreq(7 + offset), beat * 0.25, power);
  ringBuzzer (buzzer, getFreq(12 + offset), beat * 0.75, power);
  ringBuzzer (buzzer, getFreq(0 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, getFreq(-2 + offset), beat * 0.2, power);
  ringBuzzer (buzzer, 0, beat * 0.05, power);
  ringBuzzer (buzzer, getFreq(-2 + offset), beat * 0.2, power);
  ringBuzzer (buzzer, 0, beat * 0.05, power);
  ringBuzzer (buzzer, getFreq(-5 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, getFreq(2 + offset), beat * 0.5, power);
  ringBuzzer (buzzer, getFreq(0 + offset), beat * 2.5, power);
  ringBuzzer (buzzer, 0, beat * 1.25, power);
  }
  for (byte i = 0; i < 2; i++) {
  ringBuzzer (buzzer, getFreq(0 + offset), beat * 0.45, power);
  ringBuzzer (buzzer, 0, beat * 0.05, power);
  ringBuzzer (buzzer, getFreq(0 + offset), beat * 0.2, power);
  ringBuzzer (buzzer, 0, beat * 0.05, power);
  ringBuzzer (buzzer, getFreq(0 + offset), beat * 0.7, power);
  ringBuzzer (buzzer, 0, beat * 0.05, power);
  ringBuzzer (buzzer, getFreq(-5 + offset), beat * 0.2, power);
  ringBuzzer (buzzer, 0, beat * 0.05, power);
  ringBuzzer (buzzer, getFreq(-5 + offset), beat * 0.2, power);
  ringBuzzer (buzzer, 0, beat * 0.05, power);
  ringBuzzer (buzzer, getFreq(-2 + offset), beat * 0.45, power);
  ringBuzzer (buzzer, 0, beat * 0.05, power);
  ringBuzzer (buzzer, getFreq(-2 + offset), beat * 0.2, power);
  ringBuzzer (buzzer, 0, beat * 0.05, power);
  ringBuzzer (buzzer, getFreq(-2 + offset), beat * 0.2, power);
  ...
  ringBuzzer (buzzer, getFreq(0 + offset), beat * 0.2, power);
  ringBuzzer (buzzer, 0, beat * 0.05, power);
  ringBuzzer (buzzer, getFreq(0 + offset), beat * 0.25, power);
  ringBuzzer (buzzer, 0, beat * 0.5, power);
  }
  ringBuzzer (buzzer, getFreq(-12 + offset), beat * 0.45, power);
  ringBuzzer (buzzer, 0, beat * 0.05, power);
  ringBuzzer (buzzer, getFreq(-12 + offset), beat * 0.2, power);
  ringBuzzer (buzzer, 0, beat * 0.05, power);
  ringBuzzer (buzzer, getFreq(-12 + offset), beat * 0.7, power);
  ringBuzzer (buzzer, 0, beat * 0.05, power);
  ringBuzzer (buzzer, getFreq(-5 + offset), beat * 0.2, power);
  ringBuzzer (buzzer, 0, beat * 0.05, power);
  ringBuzzer (buzzer, getFreq(-5 + offset), beat * 0.2, power);
  ringBuzzer (buzzer, 0, beat * 0.05, power);
  ringBuzzer (buzzer, getFreq(-4 + offset), beat * 0.45, power);
  ringBuzzer (buzzer, 0, beat * 0.05, power);
  ringBuzzer (buzzer, getFreq(-4 + offset), beat * 0.2, power);
  ringBuzzer (buzzer, 0, beat * 0.05, power);
  ringBuzzer (buzzer, getFreq(-4 + offset), beat * 0.2, power);
  ringBuzzer (buzzer, 0, beat * 0.05, power);
  ringBuzzer (buzzer, getFreq(-2 + offset), beat * 0.2, power);
  ringBuzzer (buzzer, 0, beat * 0.05, power);
  ringBuzzer (buzzer, getFreq(-2 + offset), beat * 0.25, power);
  ringBuzzer (buzzer, 0, beat * 0.5, power);
  ringBuzzer (buzzer, getFreq(0 + offset), beat * 0.45, power);
  ringBuzzer (buzzer, 0, beat * 0.05, power);
  ringBuzzer (buzzer, getFreq(-2 + offset), beat * 0.2, power);
  ringBuzzer (buzzer, 0, beat * 0.05, power);
  ringBuzzer (buzzer, getFreq(0 + offset), beat * 1.7, power);
  ringBuzzer (buzzer, 0, beat * 0.05, power);
}
...
/* 
 *  1.7:  2019MAR28THU
 *        Updated incorrect readme documentation.
 *        Added new musics to play.
 *        Updated program initialization input methods.
 *  
 *  1.6:  2018MAR20TUE
